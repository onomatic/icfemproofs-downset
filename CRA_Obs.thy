...
lemma nil_le_skip: "seq.dnil \<le> par.dunit"
  ...
  apply (transfer; clarsimp, intro conjI; clarsimp simp: down_image_iff)
   apply (rule_tac x = "last y" in exI)
   apply (erule order_trans)
  using skip_ref_nil 
   apply (metis image_iff rangeI seq.first_last)
  using skip_ref_nil
...
lemma le_test_par: "x \<le> test xa \<parallel> test xb \<Longrightarrow> \<not> x \<le> \<bottom> \<Longrightarrow>  x \<ge> test xa" 
  by (meson dual_order.refl dual_order.trans par.down_unit par.mono_f seq.test_atom test_to_env)
...
lemma par_conv_test_inf: "par.convolute (seq.dtest p) (seq.dtest q) = (seq.dtest p \<sqinter> seq.dtest q) "
  ...
  apply (smt (verit, del_insts) order_eq_refl par.commute par.down_unit
                                 par.mono_f refine_trans test_to_env)
  
  by (metis (no_types, opaque_lifting) dual_order.trans nil_par_nil_weak 
            preorder_bot_class.bot_least seq.test_atom seq.test_le)
...
sublocale seq_par_test: 
  ...
  thm seq.assoc
         apply (clarsimp simp: seq.assoc[symmetric])
  ...
  apply (safe; clarsimp simp: in_down_iff)
   apply (metis par.aborting par.commute)
  apply (rule_tac x="\<bottom>" in bexI; clarsimp?)
   apply (rule_tac x="\<bottom>" in bexI; clarsimp?)
  apply (rule_tac x="\<bottom>" in bexI; clarsimp?)
