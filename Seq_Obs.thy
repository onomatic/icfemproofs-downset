...
locale seq_elem = seq + unital_semigroup seq last + last + 
  flip: test_semigroup "\<lambda>x y. seq y x" first + first + bot_strict seq +
  constrains first :: "'a \<Rightarrow> 'a"
  assumes first_last: "range first = range last"
begin


definition "dnil = dunit \<squnion> flip.dunit"

lemma dunit_flip: "dunit = flip.dunit" 
  by (transfer, clarsimp simp: first_last)

sublocale monoid convolute dnil
  apply (standard)
   apply (clarsimp simp: dnil_def sup_downset_def dunit_flip)
   apply (transfer)
  apply (intro set_eqI)
...
apply (clarsimp simp: dnil_def sup_downset_def dunit_flip[symmetric])
   apply (transfer)
  apply (intro set_eqI)
   apply (clarsimp simp: down_sup_distrib down_union_distrib down_image_iff)
   apply (safe; clarsimp simp: down_image_iff in_down_iff)
    apply (meson down_unit in_downsetI mono_f order_refl)
   apply (clarsimp simp: down_image_iff in_down_iff Bex_def)
  using unit_of_apply apply blast
  done
...
lemma first_test: "x \<le> test t; y \<Longrightarrow> first x \<le> test t" 
  ...
  apply (subst assoc[symmetric])
  apply (rule mono_f)
  apply (rule test_seq_test)
  by (rule order_refl)
...
lemma nil_dnil: "conv_test_pre.nil= dnil" 
  ...
  apply (safe)
   apply (clarsimp simp: downset_set_def')
   apply (metis Diff_iff bex_imageD dual_order.trans first_last in_down_iff
