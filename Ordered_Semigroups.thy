...
locale ordered_semigroup = semigroup  + lower_galois_connections + upper_galois_connections +
  constrains f :: "('a :: preorder_bot \<Rightarrow> 'a \<Rightarrow> 'a)" and bot :: 'a and less_eq :: "'a \<Rightarrow> 'a \<Rightarrow> bool"
...
locale ab_ordered_semigroup = abel_semigroup + ordered_semigroup
begin
...
locale sync_semigroup = unital_semigroup  + abel_semigroup
begin
...
sublocale sync_conv_monoid: monoid convolute dunit
  apply (standard)
  using abel_conv.commute conv_right.right_neutral apply presburger
  using abel_conv.commute conv_right.right_neutral by presburger
